...
```{r echo = FALSE, fig.width= 9, fig.height= 4, warning = F}
hist_pop_change
...
```{r echo = FALSE, fig.width= 8, warning = F}
simd_map
...
Table `r y` details the percentage of the locality's 2016 population living in the 2016 SIMD Quintiles, the percentage of the `r pop_max_year` population living in the 2020 SIMD Quintiles, and their difference for comparison. Figure `r x` then breaks down SIMD by domain in `r HSCP`.
...
In `r latest_year_access_dep`, `r format_number_for_text(access_dep_latest)`% of the population of `r HSCP` lived in the 15% most 'access deprived' areas ^3^.
...
```{r echo = FALSE, fig.width= 8.5, fig.height= 4, warning = F}
houses_ts
...
life_exp_text2 <- if (is.na(avg_life_exp_latest_male)) {
  ...
  paste0("In the latest time period available from ", latest_period_life_exp_loc, " (5-year aggregate), the average life expectancy in ", HSCP, " locality was ", avg_life_exp_latest_male, " years of age for men, and ", avg_life_exp_latest_fem, " years of age for women. A 10-year time trend can be seen in figure ", x, ".")
}
```
...
In `r latest_period_adp_presc`, `r HSCP` had a `r adp_presc_diff_scot` proportion of the population being prescribed medication for anxiety, depression, and psychosis compared to the rest of Scotland (`r format_number_for_text(adp_presc_latest)`% compared to `r format_number_for_text(scot_adp_presc)`%).
...
```{r echo=FALSE, fig.width= 8.5, fig.height = 4.5, warning=F}
alcohol_deaths_bar
...
There were **`r format_number_for_text(drug_hosp_latest)`** drug-related hospital admissions per 100,000 age-sex standardised population^4^ in `r HSCP` HSCP in the time period `r latest_period_drug_hosp` (3-year financial year aggregate). This is a `r format_number_for_text(drug_hosp_change)`% `r drug_hosp_change_word` since `r earliest_period_drug_hosp`. A trend of the change in drug-related hospital admissions for the HSCP and comparable areas is shown in figure `r x` from `r earliest_period_drug_hosp` onwards.

A comparison of areas at the most recent time period  (`r latest_period_drug_hosp` aggregated financial years) is available in figure `r x + 1` This shows `r HSCP` HSCP has a `r drug_hosp_diff_scot` rate of drug-related hospital admissions than Scotland (`r format_number_for_text(scot_drug_hosp)` admissions per 100,000).
...
```{r echo=FALSE, fig.width= 8.5, fig.height = 4.8, warning=F}
drug_hosp_bar
...
A trend of the percentage uptake of bowel screening among the eligible population is shown the HSCP and comparable areas. Data is presented as 3-year aggregates. The `r latest_period_bowel_screening` uptake rate for `r HSCP` is **`r format_number_for_text(bowel_screening_latest)`%**. This is a `r format_number_for_text(bowel_screening_change)`% `r bowel_screening_change_word` since `r earliest_period_bowel_screening`. As can be seen in figure `r x + 1`, in the latest estimate, the uptake in `r HSCP` was `r bowel_screening_diff_scot` than the uptake in Scotland overall (`r format_number_for_text(scot_bowel_screening)`%).
...
**For the most recent time periods available, `r HSCP` had:**
 ...
 - **`r hscp_emergency_adm`** emergency hospital admissions per 100,000 population, compared to `r scot_emergency_adm` in Scotland. 
 
 - **`r hscp_bed_days`** unscheduled acute specialty bed days per 100,000 population, compared to `r scot_bed_days` in Scotland.  
 
 - **`r hscp_ae_att`** A&E attendances per 100,000 population, compared to `r scot_ae_att` in Scotland. 
 
 - **`r hscp_dd`** delayed discharge bed days per 100,000 population aged over 65, compared to `r scot_dd` in Scotland. 
 ...
 - **`r hscp_ppa`** potentially preventable hospital admissions per 100,000 population, compared to `r scot_ppa` in Scotland. 
...
Information on the conditions included in PPAs is available in Appendix 3. In `r max_fy`, **`r latest_ppa_65plus`%** of PPAs in `r HSCP` were amongst those aged 65 and over, and **`r latest_ppa_under65`%** were amongst those aged under 65. 
...
**For the most recent time periods available, `r HSCP` had:**
 ...
 - **`r hscp_bed_days_mh`** unscheduled mental health specialty bed days per 100,000, compared to `r scot_bed_days_mh` in Scotland. 
...
1. Population projections are currently provided by NRS at the HSCP level. 
