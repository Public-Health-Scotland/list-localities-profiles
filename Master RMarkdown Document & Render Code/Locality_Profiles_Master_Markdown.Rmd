...
Figure `r figure_num` shows the historical population of `r HSCP`, along with the NRS population projections. `r pop_graph_text` `r pop_proj_text`. *Please see the footnotes for more information on how the population projections were calculated^2^.*
...
#### Figure `r figure_num`: Map of Data Zones within `r HSCP` coloured by SIMD quintiles.
...
Table `r table_num` details the percentage of the HSCP's 2016 population living in the 2016 SIMD Quintiles, the percentage of the `r pop_max_year` population living in the 2020 SIMD Quintiles, and their difference for comparison. Figure `r figure_num` then breaks down SIMD by domain in `r HSCP`.

#### Table `r table_num`: Percentage of the `r HSCP` population living in the 2016 and 2020 SIMD Datazone Quintiles in 2016 and `r pop_max_year` respectively.
...
#### Figure `r figure_num`: Map of services by locality in `r HSCP` HSCP^3^.
...
#### Table `r table_num`: Number of each type of service in `r HSCP` HSCP^3^.
...
#### Table `r table_num`: Breakdown of dwelling types by year for `r HSCP` HSCP.
...
#### Figure `r figure_num`: Breakdown of households by council tax band for `r HSCP` in `r max_year_housing`.
...
#### Table `r table_num`: Percentage of households by council tax band for `r HSCP` in `r max_year_housing`.
...
#### Table `r table_num`: Average life expectancy in years for the latest time periods (`r latest_period_life_exp_loc` aggregated years for the HSCP; `r latest_period_life_exp_otherareas` aggregated years for other areas).
...
```{r ltc_waffles, echo = FALSE, fig.width = 9, fig.height = 3.5, fig.cap = 'Infographic split by age group showing the number of people out of ten estimated to have at least one LTC. The age groups shown are under 65, 65-74, 75-84, and over 85.'}
ltc_waffles
```
...
#### Table `r table_num`: Multi-morbidity estimate of physical long-term conditions by age group in `r latest_year_ltc`. \newline
...
#### Figure `r figure_num`: Prevalence estimates for physical LTC by age group for `r LOCALITY` Locality. \newline

```{r ltc_types_plot, echo = FALSE, fig.width = 7.5, fig.height = 5, fig.cap = "A graph showing the prevalence estimates of physical long-term conditions by percentage of population for under 65s and for over 65s."}
ltc_types_plot
...
#### Table `r table_num`: Prevalence estimates of the five most common physical long-term conditions as a percentage of the population across areas (1 indicates the most prevalent).
...
Additionally, LTC rates are based on an adjusted population indicator in the Source Linkage Files, ensuring that the population sizes align more closely with official estimates.
