...
In `r pop_max_year`, the total population of `r HSCP` HSCP was `r total_population`. The graph below shows the population distribution of the HSCP. Overall, **`r gender_breakdown[gender_breakdown$sex == "M",]$perc`** of the population are male, and **`r gender_breakdown[gender_breakdown$sex == "F",]$perc`** are female.
...
Figure `r x` shows the historical population of `r HSCP`, along with the NRS population projections. `r pop_graph_text` `r pop_proj_text`. *Please see the footnotes for more information on how the population projections were calculated^1^.*
...
For the most recent time period available^3^, `r HSCP` HSCP had:
...
For the most recent time period available^3^, `r HSCP` had:
