...
palette <- phsstyles::phs_colours(c("phs-purple", "phs-magenta", "phs-blue", "phs-green", 
...
format_number_for_text <- function(x){
  ...
  x <- ifelse(abs(x) < 1, round_half_up(x,2),           # if x < 1 then show 2dp
              ifelse(abs(x) < 100, round_half_up(x,1),   # if 1 =< x < 100 then 1dp
                    round_half_up(x)))            # if 10 =< x then no decimal places
...
test <-  data <- fs::dir_ls(glue("/conf/linkage/output/lookups/Unicode/",
...
read_in_dz_pops <- function() {
  ...
  fs::dir_ls(glue("/conf/linkage/output/lookups/Unicode/",
...
proj <-  fs::dir_ls(glue("/conf/linkage/output/lookups/Unicode/",
...
clean_scotpho_dat <- function(data){
  ...
  data %>% 
    ...
    mutate(area_type = if_else(area_type == "HSC partnership", "HSCP", area_type),
...
scotpho_time_trend <- function(data, chart_title, xaxis_title, yaxis_title, string_wrap, rotate_xaxis = F){
  ...
  data %>% 
    ...
    # plot
    ggplot(aes(x = str_wrap(period_short, width = string_wrap), y = measure,
    ...
    geom_ribbon(aes(x = str_wrap(period_short, width = string_wrap), 
...
ptsp <- function(partnership){
  recode(partnership, "Borders" = "Scottish Borders", 
...
hbres <- function(hbres_currentdate){
  case_when(hbres_currentdate == "S08000015" ~ "NHS Ayrshire & Arran",
