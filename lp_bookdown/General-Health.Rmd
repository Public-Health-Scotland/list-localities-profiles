...
```{r}
ftext(
  glue("Table {figure_num}: Average life expectancy in years for the latest time periods ({latest_period_life_exp_loc} aggregated years for the locality; {latest_period_life_exp_otherareas} aggregated years for other areas)."),
  table_heading_style
)
...
-   Figure `r figure_num` shows the Long-Term condition prevalence estimates for `r LOCALITY` Locality in the financial year `r latest_year_ltc` across all ages.

-   Table `r table_num` orders the prevalence estimates to show the five most common physical LTCs across all ages at the Locality, Partnership, and Scotland levels.

Below is a breakdown of the physical LTCs, for the financial year `r latest_year_ltc`. Figure `r figure_num` shows the prevalence of different LTCs in `r LOCALITY` locality, and Table `r table_num` illustrates the five most prevalent physical LTCs across all ages at the locality, partnership, and Scotland levels.

```{r, fig.width = 7.5, fig.height = 5, fig.cap = "A graph showing the prevalence of physical long term conditions by percentage of population."}
ftext(paste0("Figure ", figure_num, ": The percentage of people with each physical LTC."), table_heading_style)
