...
pops <- populations %>% 
  select(year, hb2019name, hscp2019name,
...
ppa_areas <- ppa %>% 
  ...
  left_join(pop_areas_all_ages) %>% 
  mutate(data = round_half_up(n/pop*100000)) #%>% 
  #mutate(location = factor(location, levels = c(HSCP, HB, "Scotland"))) %>% #error message here 
  #arrange(location)
