...
life_exp_trend <- life_exp %>% 
  ...
  filter(area_name == HSCP & area_type == "HSCP" & year >= max(year)-10) %>% 
  ...
  theme_profiles()+
  expand_limits(y = 0)+
  labs(title = paste0("Average Life Expectancy in ", str_wrap(`HSCP`, 40)), 
...
life_exp_table <- life_exp %>%
  ...
  mutate(measure = round_half_up(measure, 1)) %>% 
  mutate(area_type = factor(area_type, levels = c("HSCP", "Health board", "Scotland")),
...
life_exp_table <- life_exp %>%
  ...
  select("Sex" = sex, area_name, area_type, measure) %>% 
  mutate(measure = round_half_up(measure, 1)) %>% 
  mutate(area_type = factor(area_type, levels = c("Locality", "HSCP", "Health board", "Scotland")),
         area_name = fct_reorder(as.factor(area_name), as.numeric(area_type))) %>% 
...
##Numbers for text
avg_life_exp_latest_male <- if(HSCP %in% check_missing_data_scotpho(life_exp)$area_name){
  NA
} else {
...
avg_life_exp_latest_fem <- if(HSCP %in% check_missing_data_scotpho(life_exp)$area_name){
  NA
} else {
  round_half_up(filter(
...
##Numbers for text
deaths_15_44_latest <- filter(deaths_15_44, 
                              year == max(deaths_15_44$year) & 
                                (area_name == HSCP & area_type == "HSCP"))$measure

scot_deaths_15_44 <- filter(deaths_15_44, 
                            year == max(deaths_15_44$year) & area_name == "Scotland")$measure
...
##Numbers for text
cancer_reg_rate_latest <- filter(cancer_reg, 
                                 year == max(cancer_reg$year) & 
                                   (area_name == HSCP & area_type == "HSCP"))$measure

cancer_reg_total_latest <- filter(cancer_reg, 
                                  year == max(cancer_reg$year) & 
                                    (area_name == HSCP & area_type == "HSCP"))$numerator


### Early deaths from cancer

latest_period_early_deaths_cancer <- unique(filter(early_deaths_cancer, 
                                                   year == max(early_deaths_cancer$year))$period_short)

early_deaths_cancer_rate_latest <- filter(early_deaths_cancer, 
...
disease_hosp <- bind_rows(
  ...
  mutate(indicator = case_when(str_detect(indicator, "Asthma") ~ "Asthma",
...
adp_presc_bar <- adp_presc %>% 
  scotpho_bar_chart_HSCP(data = .,
                         chart_title = paste0("Anxiety, Depression and Psychosis Prescriptions, ", 
                                              max(.$period_short)),
...
adp_presc_latest <- filter(adp_presc, 
...
scot_adp_presc <- filter(adp_presc, 
                         year == max(adp_presc$year) & area_name == "Scotland")$measure
...
slf_pops <- ltc %>% 
  group_by(age_group, hscp2019name, hscp_locality) %>% 
  summarise(slf_adj_pop = first(slf_adj_pop)) %>% 
  ungroup()

slf_pop_loc <- slf_pops %>% 
  group_by(age_group,hscp2019name)%>%
...
create_infographic <- function(image1, image2, perc_ltc, col, age_label1, age_label2){
  ...
  ggplot() + 
    ...
    geom_rect(data = data.frame(x1=0, x2=1, y1=0, y2=1.3),
...
ltc_infographic <- ltc %>% 
  filter(hscp2019name == HSCP) %>% 
  filter(total_ltc > 0) %>% 
  select(hscp2019name, age_group, total_ltc, people) %>% 
...
waffle.u65 <- create_infographic(image1 = ppl_faint_u65, image2 = ppl_bold_u65, perc_ltc = ltc.percent.u65,
                                 col = palette[1], age_label1 = "under 65", age_label2 = "UNDER 65")

waffle.6574 <- create_infographic(image1 = ppl_faint_6574, image2 = ppl_bold_6574, perc_ltc = ltc.percent.6574,
                                  col = palette[2], age_label1 = "65 to 74", age_label2 = "65 - 74")

waffle.7584 <- create_infographic(image1 = ppl_faint_7584, image2 = ppl_bold_7584, perc_ltc = ltc.percent.7584,
                                  col = palette[3], age_label1 = "75 to 84", age_label2 = "75 - 84")

waffle.o85 <- create_infographic(image1 = ppl_faint_o85, image2 = ppl_bold_o85, perc_ltc = ltc.percent.o85,
...
rm(ppl_bold_u65, ppl_faint_u65, ppl_faint_o85, ppl_bold_o85, 
...
## Figures for text
ltc_multimorbidity_un65_perc <- sum(filter(ltc_multimorbidity, 
                                           total_ltc != "1 LTC",
                                           age_group == "Under 65")$percent)

ltc_multimorbidity_ov65_perc <- sum(filter(ltc_multimorbidity, 
                                           total_ltc != "1 LTC",
                                           age_group == "65+")$percent)
...
lims.un65 <- case_when(max(ltc_types$percent) < 20 ~ -10,
                       between(max(ltc_types$percent), 20, 24) ~ -12,
                       max(ltc_types$percent) > 24 ~ -15)
lims.ov65 <- case_when(max(ltc_types$percent) < 20 ~ 20,
...
ltc_plot_left <- 
  ...
  theme_profiles()+
  theme(title = element_text(colour = palette[1]),
...
title <- ggdraw()+
  draw_label(paste0("Prevalence of Physical Long-Term Conditions in\n", 
                    `HSCP`, " HSCP"), size = 11, fontface = "bold")
...
rm(ltc_plot_left, ltc_axis, ltc_plot_right, 
...
#ltc_totals <- left_join(ltc_totals, select(lookup, hscp2019name)) #join df with lookup2
...
ltc_cols <- ltc_scot %>%
  ...
  rename(topltc = key) %>% 
  arrange(desc(value)) %>% 
  mutate(colours = c(palette, c("navy", "lightsalmon4", "deeppink4",
...
#ltc_loc_col <- tableGrob(top5ltc_loc[,1], cols = loc.ltc.table, rows = c(1:5),
...
ltc_hscp_col <- tableGrob(top5ltc_hscp[,1], cols = hscp.ltc.table,
                          theme=ttheme_default(core=list(bg_params = list(fill=top5ltc_hscp$colours),fg_params=list(col = "white",fontface=2, fontsize = 11)),
                                               colhead=list(bg_params = list(fill = "white"), fg_params=list(fontface=3, fontsize = 11))))
ltc_scot_col <- tableGrob(top5ltc_scot[,1], cols = "Scotland",
                          theme=ttheme_default(core=list(bg_params = list(fill=top5ltc_scot$colours),fg_params=list(col = "white", fontface=2, fontsize = 11)),
                                               colhead=list(bg_params = list(fill = "white"), fg_params=list(fontface=3, fontsize = 11))))

## Combine columns
TOPltcs <- gtable_combine(ltc_hscp_col, ltc_scot_col)
...
rm(ltc_cols, ltc_loc_col, ltc_hscp_col, ltc_scot_col,
