...
## File path
filepath <- paste0("/conf/LIST_analytics/West Hub/02 - Scaled Up Work/",
...
pops <- pop_raw_data %>% 
  select(year, sex, hscp2019name, hscp_locality,
...
## Gender
gender_breakdown <- pops %>% filter(hscp_locality == "Partnership Total",
                                    hscp2019name == HSCP,
                                    year == max(year)) %>%
  select(sex, total_pop) %>%
  mutate(total = sum(total_pop),
...
pop_pyramid <- ggplot(pop_breakdown, 
                      aes(x = factor(Age, levels = unique(pop_breakdown$Age)), 
                          fill = Gender)) +
  geom_bar(data = subset(pop_breakdown, Gender == "Male"), stat = "identity",
           aes(y=Population)) + 
  geom_bar(data = subset(pop_breakdown, Gender == "Female"), stat = "identity",
           aes(y=Population*(-1))) + 
  ...
  labs(y = "Population", x = "Age Group",
       title = paste0(str_wrap(`HSCP`, 50), " population pyramid")) 
...
hist_pop_change <- ggplot(hist_pop_breakdown, 
                          aes(x = factor(Age, levels = ord), 
  ...
  labs(x = "Age Group", y = "Percent Change",
...
hscp_pop_proj_weight <- hscp_pop_proj %>% 
  ...
  mutate(age_group = case_when(age %in% 0:4 ~ "Pop0_4",
                               age %in% 5:17 ~ "Pop5_17",
...
pop_plot_dat <- rbind(clean_names(mutate(locality_pop_trend, data = "HISTORICAL")),
...
pop_latest <- locality_pop_trend[locality_pop_trend$year == max(locality_pop_trend$year),]$pop 
pop_last <- locality_pop_trend[locality_pop_trend$year == max(locality_pop_trend$year) - 1,]$pop

# if there is no linear trend, this calculates the year of the last change point
change_point <- locality_pop_trend %>% 
  mutate(change = ifelse(lag(pop) > pop, 1, 0), 
...
pop_graph_text <- ifelse(pval < 0.05, 
                         ...
                         paste0("The population has been ", 
                         ...
                         paste0(
                           paste("There is no significant linear trend in population.",
...
pop_proj_text <- paste("The population in", HSCP, "is estimated to", 
...
rm(reg, pval, coef, pop_latest, pop_last, change_point, pop_change, pop_proj_change, 
