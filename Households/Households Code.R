...
# aggregate data
house_dat1 <- house_dat %>% dplyr::group_by(year) %>%
  dplyr::summarise(total_dwellings = sum(total_number_of_dwellings),
...
houses_ts <- ggplot(house_dat1, aes(x = year, y = total_dwellings, group = 1)) + 
  ...
  labs( x = "Year", y = "Number of Dwellings",
        title = paste0("Number of Dwellings by Year in ", str_wrap(`LOCALITY`, 40)), 
...
ctb <- house_dat2 %>% select((council_tax_band_a:council_tax_band_h)) %>% melt()

variable = ctb$variable

pal_ctb <- phsstyles::phs_colours(c("phs-magenta", "phs-magenta-80", "phs-magenta-50", "phs-magenta-10",
...
ctb_plot <- ggplot(ctb, aes(fill=factor(variable, levels = rev(variable)), y=value, x = 1)) +
  ...
  scale_fill_manual(name = "Council Tax Band", 
...
ctb_table <- ctb %>% mutate(percent = paste0(format_number_for_text(100*value/sum(value)),"%")) %>%
  select(-value) %>%
  spread(variable, percent) %>%
  mutate(`Tax Band` = "Percent of households") %>%
  select(`Tax Band`, A = council_tax_band_a, B = council_tax_band_b, 
...
perc_houses_AC <- format_number_for_text(sum(house_dat2$council_tax_band_a, 
                                             house_dat2$council_tax_band_b, 
                                             house_dat2$council_tax_band_c)/house_dat2$total_number_of_dwellings*100)

perc_houses_FH <- format_number_for_text(sum(house_dat2$council_tax_band_f, 
                                             house_dat2$council_tax_band_g, 
                                             house_dat2$council_tax_band_h)/house_dat2$total_number_of_dwellings*100)
