...
zones<-readOGR(dsn = paste0("//conf/linkage/output/lookups/Unicode/Geography/Shapefiles/Data Zones 2011/"),
...
# plot
simd_map <- ggplot(data = zones_tidy, 
                   aes(x = long, y = lat)) + 
...
## deprivation domains
plot_labels <- c("Access", "Crime", "Education", "Employment", 
...
simd_domains <- simd2020 %>% 
  ...
  ggplot(aes(fill=factor(value, levels = 1:5), y = total_pop, 
...
simd2016_dom <- simd2016_dom %>% 
  ...
  dplyr::summarise(pop = sum(pop_16), 
                   total_pop = max(total_pop)) %>%
  dplyr::mutate(perc_16 = pop/total_pop, 
...
simd2020_dom <- simd2020_dom %>% 
  ...
  dplyr::group_by(variable, value) %>%
  dplyr::summarise(pop = sum(pop_20), 
                   total_pop = max(total_pop)) %>%
  dplyr::mutate(perc_20 = pop/total_pop, 
                domain = gsub("_20", "", variable)) %>%
  dplyr::ungroup() %>%
  dplyr::select(domain, perc_20, quintile = value)

domains <- simd2020_dom$domain %>% unique()
base_data <- tibble(domain = rep(domains, each = 5),
                    quintile = rep(1:5,8))
...
simd_16_20_dom <- full_join(base_data, simd2016_dom) %>% 
  ...
  mutate(domain = ifelse(domain == "simd", "SIMD", tools::toTitleCase(domain)),
...
simd_diff_plot <- ggplot(simd_16_20_dom, aes(x = quintile, y = diff, fill = factor(quintile))) + 
  ...
  geom_bar(stat = "identity", position = position_dodge(),
           color = "black") + 
  geom_text(aes(label=paste0(format(janitor::round_half_up(100*diff,1),nsmall = 1),"%"),
                vjust = v_just), color="black",
            position = position_dodge(0), size=2.8) +
  scale_y_continuous(labels = scales::percent, 
                     limits = c(-1.4*max(abs(simd_16_20_dom$diff)), 1.4*max(abs(simd_16_20_dom$diff)))) +
  scale_fill_manual(name = "Population-Weighted Quintile",
                    labels = simd_cats,
                    values =  (simd_col), drop = F) +
  theme_minimal()+
  theme(legend.position = "bottom")+
  labs(x = "", y = "Difference from 2016",
       title = paste0("Difference in Population Living in Deprivation Quintiles by SIMD Domain\n", 
...
simd_diff_overall <- simd_16_20_dom %>% 
  filter(domain == "SIMD")  %>% 
  mutate(Quintile = paste(domain, quintile),
