...
colnames(demographic_values) <- c(str_wrap("Demographic Indicators", width = 35), headings)
...
colnames(housing_values) <- c(str_wrap("Housing Indicators", width = 35), headings)
...
colnames(general_health_values) <- c(str_wrap("General Health Indicators", width = 35), headings)
...
colnames(lifestyle_values) <- c(str_wrap("Lifestyle and Risk Factors Indicators", width = 35), headings)
...
# Values
loc_values <- format(
  c(latest_emergency_adm_loc$n, latest_bed_days_loc$n, latest_ae_att_loc$n, latest_dd_loc$n, latest_ppa_loc$n[2]),
  big.mark = ","
)
other_loc_values <- bind_rows(other_loc_emergency_adm, other_loc_bed_days, other_loc_ae_att, other_loc_dd, other_loc_ppa)
hscp_values <- format(
  c(hscp_emergency_adm$n, hscp_bed_days$n, hscp_ae_att$n, hscp_dd$n, hscp_ppa$n[2]),
  big.mark = ","
)
scot_values <- format(
  c(scot_emergency_adm$n, scot_bed_days$n, scot_ae_att$n, scot_dd$n, scot_ppa$n[2]),
  big.mark = ","
)
...
colnames(usc_values) <- c(str_wrap("Hospital and Community Care Indicators", width = 35), headings)
...
hscp_values <- c(hscp_psych_hosp$measure2[2], format(hscp_bed_days_mh$n, big.mark = ","))
scot_values <- c(scot_psych_hosp$measure2[2], format(scot_bed_days_mh$n, big.mark = ","))
...
colnames(usc_mh_values) <- c(str_wrap("Hospital Care (Mental Health) Indicators", width = 35), headings)
