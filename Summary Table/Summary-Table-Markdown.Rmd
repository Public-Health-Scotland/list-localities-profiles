...
# Values
loc_values <- format(
  c(latest_emergency_adm_loc1, latest_bed_days_loc1, latest_ae_att_loc1, latest_dd_loc1, filter(latest_ppa_loc, year == max(year))$data),
  big.mark = ","
)
...
loc_values <- c(
  ...
  format(
    ifelse(is_empty(latest_bed_days_mh_loc$data), NA_real_, latest_bed_days_mh_loc1),
    big.mark = ","
  )
)
