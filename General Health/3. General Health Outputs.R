...
life_exp_trend <- life_exp %>% 
  ...
  theme_profiles()+
  expand_limits(y = 0)+
  labs(title = paste0("Average Life Expectancy in ", str_wrap(`LOCALITY`, 40)), 
  ...
  NA
    ...
    round_half_up(filter(
      life_exp, sex == "Male" & year == latest_year_life_exp_loc &
        area_name == LOCALITY & area_type == "Locality")$measure,1)
  } 
...
cancer_reg_total_latest <- filter(cancer_reg, 
                                  year == max(cancer_reg$year) & 
                                    (area_name == LOCALITY & area_type == "Locality"))$numerator
...
title <- ggdraw()+
  draw_label(paste0("Prevalence of Physical Long-Term Conditions in\n", 
                    `LOCALITY`, " Locality"), size = 11, fontface = "bold")
