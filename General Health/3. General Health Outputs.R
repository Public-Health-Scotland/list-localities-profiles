...
title <- ggdraw() +
  draw_label(
    str_wrap(
      glue("Prevalence estimates for {latest_year_ltc} of Physical Long-Term Conditions in the {LOCALITY} Locality"),
      width = 65
    ),
    size = 11,
    fontface = "bold"
...
caption <- ggdraw() +
  draw_label("Source: SPARRA via the Source Linkage Files", size = 10, hjust = -0.5)
...
title <- ggdraw() +
  draw_label(str_wrap(
    glue("Top 5 most common Physical Long-Term Conditions in {LOCALITY} Locality by prevelance estimates for {latest_year_ltc}, compared to {HSCP} HSCP and Scotland estimates."),
    width = 65
