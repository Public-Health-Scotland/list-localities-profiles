...
import_folder <- "/conf/LIST_analytics/Orkney/Analysts/CF/IZ_profiles/Unscheduled Care/DATA 2022/"
...
localities <- read_in_localitiesiz()
...
other_locs <- lookup %>% 
  filter(hscp_locality %in% c("Mull, Iona, Coll and Tiree","Whisky Isles","Bute","Rothesay Town")) |> 
  select(hscp_locality, hscp2019name) %>% 
  filter(hscp2019name == HSCP & hscp_locality != LOCALITY) %>% 
  arrange(hscp_locality)
...
pops <- populations %>% 
  select(year, hb2019name, hscp2019name, hscp_locality,
...
pop_areas <- pops %>% 
  ...
  mutate(age_group = case_when(age_group == "Pop0_17" ~ "0 - 17",
...
EAs_age_ts <- age_group_trend_usc(data_for_plot = emergency_adm_age,
...
EAs_loc_ts <- area_trend_usc(data_for_plot = emergency_adm_areas,
...
bd_patch <- readRDS(paste0(import_folder, "bd_patch.rds")) %>%
...
BDs_age_ts <- age_group_trend_usc(data_for_plot = bed_days_age,
...
BDs_loc_ts <- area_trend_usc(data_for_plot = bed_days_areas,
...
BDMH_age_ts <- age_group_trend_usc(data_for_plot = bed_days_mh_age,
...
BDMH_loc_ts <- area_trend_usc(data_for_plot = bed_days_mh_areas,
...
AandE_age_ts <- age_group_trend_usc(data_for_plot = ae_att_age,
...
AandE_loc_ts <- area_trend_usc(data_for_plot = ae_att_areas,
...
#delayed_disch <- readRDS(paste0(import_folder, "delayed_discharges_msg.rds")) %>% 
 ...
 # summarise(dd_people = sum(dd_people),
...
#DD_loc_ts <- area_trend_usc(data_for_plot = delayed_disch_areas,
...
Falls_loc_ts <- area_trend_usc(data_for_plot = falls_areas,
...
readmissions_age <- readmissions %>%
  ...
  drop_na(age_group) %>% 
  group_by(financial_year, age_group) %>% 
  summarise(read_28 = sum(read_28),
            discharges = sum(discharges)) %>%
  ungroup() %>%
  mutate(data = round_half_up(read_28/discharges*1000, 1))

Read_age_ts <- age_group_trend_usc(data_for_plot = readmissions_age,
...
Read_loc_ts <- area_trend_usc(data_for_plot = readmissions_areas,
...
PPA_loc_ts <- area_trend_usc(data_for_plot = ppa_areas,
...
#psych_hosp_latest <- round_half_up(filter(psych_hosp, 
