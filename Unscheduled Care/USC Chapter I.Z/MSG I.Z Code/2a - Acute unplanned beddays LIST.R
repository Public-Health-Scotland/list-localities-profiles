...
smr01_extract <- as_tibble(dbGetQuery(channel, statement = "SELECT LINK_NO, ADMISSION_DATE,
                                      ...
                                      WHERE DISCHARGE_DATE >= TO_DATE('2017-04-01','YYYY-MM-DD') AND HBRES_KEYDATE = 'S08000022'")) %>%
...
smr_cis <- smr_dt[, .(
  cis_admission_date = min(admission_date),
  cis_discharge_date = max(discharge_date),
  cis_ca2019 = first(ca2019),
  cis_admission_type = first(admission_type),
  cis_intzone2011 = first(intzone_2011),
  cis_datazone2011 = first(datazone2011)
...
smr01_beddays <- smr01_final %>%
  select(
    -cis_admission_date, -cis_discharge_date, -admission_type, -dr_postcode,
...
list_final <- smr01_beddays %>%
  ...
  dtplyr::lazy_dt() %>%
  group_by(
    council_area2019name, cis_intzone2011, area_treated, location, specialty,
    significant_facility, age_group, month_year
  ) %>%
  summarise(unplanned_beddays = sum(beddays)) %>%
  ungroup() %>%
  tibble::as_tibble() %>%
  rename(council = "council_area2019name", intzone2011 = "cis_intzone2011", month = "month_year") %>%
...
arrow::write_parquet(list_final, path(data_folder, "2a-Acute-Beddays-breakdown.parquet"), compression = "zstd")
