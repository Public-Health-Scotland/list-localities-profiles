...
smr_falls <- smr1_extract %>%
  ...
  mutate(fall_flag = purrr::pmap_dbl(
  ...
  filter(fall_flag == 1) %>%
  select(-fall_flag) %>%
  rename(datazone2011 = datazone_2011, intzone2011 = intzone_2011, age = age_in_years) %>%
...
smr1_extract_read <- smr1_extract %>%
  ...
  arrange(
    link_no, cis_marker, admission_date,
...
ppa_id <- smr1_extract %>%
  ...
  mutate(
    op1a13 = replace_na(op1a13, "0"), d1c14 = replace_na(d1c14, "0"), d2c14 = replace_na(d2c14, "0"),
