...
emergency_adm <- read_parquet(path(import_folder_southayrshire, "emergency_admissions_msg.parquet")) %>%
...
bed_days <- read_parquet(path(import_folder_southayrshire, "bed_days_msg.parquet")) %>%
  filter(financial_year <= max_fy)
...
bed_days_mh <- read_parquet(path(import_folder_southayrshire, "bed_days_mh_msg.parquet")) %>%
  filter(financial_year <= max_fy)
...
ae_attendances <- read_parquet(path(import_folder_southayrshire, "ae_attendances_msg.parquet")) %>%
  filter(financial_year <= max_fy)
...
falls <- read_parquet(path(import_folder_southayrshire, "falls_smr.parquet")) %>%
  filter(financial_year <= max_fy) %>%
  filter(age_group %in% c("65 - 74", "75+"))
...
readmissions <- read_parquet(path(import_folder_southayrshire, "readmissions_smr.parquet")) %>%
  filter(financial_year <= max_fy)
