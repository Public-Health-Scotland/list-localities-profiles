...
aggregate_area_data <- function(data, measure) {
  data %>%
    ...
    mutate(
      level = factor(level, levels = c("Locality", "HSCP", "HB", "Scotland"))
    )
}
  ...
  indicator_loc_ts <- indicator_areas %>%
    ...
    area_trend_usc(
      ...
      plot_title = if_else(denominator_number != 100000 | denominator_name != "population",
        paste(indicator_name, "per", format(denominator_number, big.mark = ",", scientific = FALSE), denominator_name, "over time by residence"),
      ...
      yaxis_title = if_else(denominator_number != 100000 | denominator_name != "population",
        paste(if_else(indicator_name == "Potentially Preventable Admissions (PPA)", "PPA", indicator_name), "\nper", format(denominator_number, big.mark = ",", scientific = FALSE), denominator_name),
        paste(if_else(indicator_name == "Potentially Preventable Admissions (PPA)", "PPA", indicator_name), "rate\n per 100,000 population")
      ),
...
scotpho_usc_charts_and_text <- function(
  dataset,
  indicator_name,
  denominator_number,
  denominator_name,
...
emergency_adm_age_outputs <- usc_age_level_charts_and_text(
  emergency_adm,
  adm,
  "Emergency Admissions",
  100000,
...
bed_days_age_outputs <- usc_age_level_charts_and_text(
  bed_days,
  bd,
  "Unscheduled Bed Days",
  100000,
...
bed_days_mh_age_outputs <- usc_age_level_charts_and_text(
  bed_days_mh,
  bd,
  "Unscheduled Mental Health Bed Days",
  100000,
...
falls_age_outputs <- usc_age_level_charts_and_text(
  falls,
  adm,
  "Emergency Admissions From Falls",
  100000,
...
ppa_age_outputs <- usc_age_level_charts_and_text(
  ppa,
  adm,
  "Potentially Preventable Admissions (PPA)",
  100000,
...
psych_hosp_outputs <- scotpho_usc_charts_and_text(
  psych_hosp,
