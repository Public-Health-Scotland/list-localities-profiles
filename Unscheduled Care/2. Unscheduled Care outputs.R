...
aggregate_area_data <- function(data,measure){
  ...
  summarise(
    ...
    {{measure}}:=sum({{measure}}),
    .by=c("year","financial_year","hb2019name","hscp2019name","hscp_locality","age_group")
    ...
    bind_rows(
      ...
      summarise(
        .,
        {{measure}}:=sum({{measure}}),
        .by=c("year","financial_year","hb2019name","hscp2019name","age_group")
    ...
    dplyr::select(year,financial_year,hb2019name,hscp2019name,hscp_locality,age_group,location,level,{{measure}}) %>%
    mutate(
      level = factor(
        level,
...
populations_filtered <- populations %>%
  ...
  dplyr::select(year,
                ...
                hscp2019name,
                hscp_locality,
                datazone2011,datazone2011name,
                sex,
                Pop0_17,Pop18_44,Pop45_64,Pop65_74,Pop75Plus,
                Pop65Plus,
                total_pop) %>%
  pivot_longer(cols=c("Pop0_17","Pop18_44","Pop45_64","Pop65_74","Pop75Plus","Pop65Plus","total_pop"),
               names_to = "age_group",values_to="pop") %>%
  mutate(
    age_group = case_when(
...
populations_filtered_age <- populations_filtered %>%
  ...
  summarise(pop=sum(pop),.by=c("year","financial_year","hb2019name","hscp2019name","hscp_locality","age_group")) 
...
area_trend_usc <- function(data_for_plot, measure, plot_title, yaxis_title, source) {
  data_for_plot <- data_for_plot %>%
    rename(n=measure) %>%
    mutate(
...
unscheduled_care_charts_and_text <- function(dataset,
                                             indicator_column,
                                             indicator_name,
                                             denominator_number,
                                             denominator_name,
  ...
  indicator_areas <- dataset %>%
    ...
    summarise({{indicator_column}}:=sum({{indicator_column}}),
              .by=c("year","financial_year","hb2019name","hscp2019name","hscp_locality","location","level")) %>%
    left_join(
      populations_area,
      by = c("year","financial_year","hb2019name","hscp2019name","hscp_locality","location","level")
    ) %>%
    mutate(rate = round_half_up(100000 *({{indicator_column}} / pop))) %>%
    dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,location,level,{{indicator_column}},pop,rate) 
  
  
  indicator_loc_ts <- indicator_areas %>%
    filter(level == "Locality" & location == LOCALITY | 
  ...
  percentage_change_areas <- indicator_areas %>%
    ...
    dplyr::select(financial_year,hb2019name,hscp2019name,hscp_locality,location,level,rate) %>%
    ...
    filter(level == "Locality" & location %in% c(LOCALITY,other_locs) | 
             level == "HSCP" & location == HSCP |
             level == "HB" & location == HB |
             level == "Scotland") %>%
    ...
    mutate(text = case_when(
      ...
      level == "Locality" ~ paste0(
        ...
        str_to_lower(indicator_name),
        " rate ",
        if_else(denominator_number != 100000 | denominator_name != "population",
                paste0("per ",format(denominator_number,big.mark=",",scientific = FALSE)," "),
                ""),
        if_else(denominator_number != 100000 | denominator_name != "population",
                paste0(denominator_name," "),
                ""),
    ...
    filter((level == "Locality" & location == LOCALITY) | level %in% c("HSCP","HB","Scotland")) 
  ...
  indicator_paragraph_area <- paste0(
    ...
    filter(percentage_change_areas,level == "HB")$text," and ",
  ...
  if (indicator_name == "Potentially Preventable Admissions (PPA)"){
    ...
    indicator_paragraph_area <- paste0(toupper(substring(indicator_paragraph_area,1,1)),
                                       substring(indicator_paragraph_area,2,nchar(indicator_paragraph_area))) # Make first letter a capital
    
    
    indicator_paragraph_area <- gsub("potentially preventable admissions \\(ppa\\)","PPA",indicator_paragraph_area)
  ...
  indicator_age <- dataset %>%
    ...
    mutate(rate = round_half_up(100000*({{indicator_column}} / pop))) %>%
    dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,age_group,level,{{indicator_column}},pop,rate) 
  
  if (indicator_name == "Potentially Preventable Admissions (PPA)"){
    
    ppa_agebanded <- indicator_age %>%
      mutate(age_group_banded = case_when(
        age_group %in% c("0 - 17", "18 - 44","45 - 64") ~ "Under 65",
        age_group %in% c("65 - 74", "75+") ~ "65+")
      ) %>%
      summarise({{indicator_column}}:=sum({{indicator_column}}),
                pop=sum(pop),
                .by=c("year","financial_year","hb2019name","hscp2019name","hscp_locality","level",
                      "age_group_banded")) %>%
      group_by(year,financial_year,hb2019name,hscp2019name,hscp_locality,level) %>%
      mutate(perc = round_half_up(100*({{indicator_column}} / sum({{indicator_column}})), 1)) %>%
      ungroup()
  ...
  indicator_age_ts <- indicator_age  %>%
    age_group_trend_usc(
      measure="rate",
      plot_title = paste(
        indicator_name,"per 100,000 over time by age group\n for",
        LOCALITY
      ),
      yaxis_title = paste(indicator_name,"rate\n per 100,000 population"),
      source = source
  ...
  percentage_change_age <- indicator_age %>%
    filter(financial_year %in% c(min_fin_year,max_fin_year)) %>%
    dplyr::select(financial_year,hb2019name,hscp2019name,hscp_locality,level,age_group,rate) %>%
    pivot_wider(names_from=financial_year,values_from=rate) %>%
    mutate(rate_change=!!sym(max_fin_year)-!!sym(min_fin_year)) %>%
    mutate(perc_change=100*(abs(rate_change)/!!sym(min_fin_year))) %>%
    mutate(perc_change=round_half_up(perc_change,digits=1))  %>%
    
    mutate(rate_ranking = case_when(
...
scotpho_unscheduled_care_charts_and_text <- function(dataset,
                                                     indicator_name,
                                                     denominator_number,
                                                     denominator_name,
                                                     xaxis_title,
  ...
  percentage_change_data <- dataset %>%
    ...
    mutate(text = case_when(
      ...
      area_type == "Locality" ~ paste0(
        ...
        max_period_for_text, 
        " is ", 
        format(!!sym(max_period),big.mark=","),", ", get_article(perc_change),
        " ",
        perc_change,
        "% ", 
  ...
  indicator_paragraph <- paste0(
    ...
    filter(percentage_change_data,area_type == "Locality")$text," and ",
    filter(percentage_change_data,area_type == "HSCP")$text,". ",
    filter(percentage_change_data,area_type == "Health board")$text," and ",
...
emergency_adm <- read_parquet(paste0(
  ...
  left_join(
    ...
    by = c("financial_year","hscp2019name","hscp_locality","age_group","level")
  ) %>%
  mutate(rate = round_half_up(admissions / pop * 100000)) %>%
  dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,age_group,adm=admissions,pop,rate,level) 


emergency_adm_outputs <- unscheduled_care_charts_and_text(emergency_adm,
                                                          adm,
...
bed_days <- read_parquet(paste0(import_folder, "bed_days_msg.parquet")) %>%
  ...
  dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,age_group,bd=bed_days,pop,rate,level) 


bed_days_outputs <- unscheduled_care_charts_and_text(bed_days,
                                                     bd,
...
bed_days_mh_outputs <- unscheduled_care_charts_and_text(bed_days_mh,
                                                        bd,
...
ae_attendances <- read_parquet(paste0(
  ...
  dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,age_group,att=attendances,pop,rate,level) 

ae_attendances_outputs <- unscheduled_care_charts_and_text(ae_attendances,
                                                           att,
                                                           "A & E Attendances",
                                                           100000,
                                                           "population",
...
delayed_disch <- read_parquet(paste0(
  ...
  dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,level,age_group,
                dd_ppl=dd_people,dd_bd=dd_bed_days,
                dd_bd_rate,dd_ppl_rate,pop)


delayed_discharges_outputs <- unscheduled_care_charts_and_text(delayed_disch,
                                                               dd_bd,
...
falls <- read_parquet(paste0(import_folder, "falls_smr.parquet")) %>%
  ...
  dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,level,
                age_group,
                adm=admissions,adm_rate,pop)



falls_outputs <- unscheduled_care_charts_and_text(falls,
                                                  adm,
...
readmissions <- read_parquet(paste0(
  ...
  filter(!is.na(year)) %>%
  mutate(dd_rate = round_half_up(discharges / pop * 100000),
         read_28_rate = round_half_up(read_28 / pop * 100000)) %>%
  dplyr::select(financial_year,year,hb2019name,hscp2019name,hscp_locality,level,age_group,
                dd=discharges,read_28,dd_rate,read_28_rate,pop)

readmissions_outputs <- unscheduled_care_charts_and_text(readmissions,
                                                         dd,
...
ppa_outputs <-  unscheduled_care_charts_and_text(ppa,
                                                 adm,
...
psych_hosp_outputs <- scotpho_unscheduled_care_charts_and_text(psych_hosp,
