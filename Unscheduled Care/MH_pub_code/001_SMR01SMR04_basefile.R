...
### 3 - Set filepaths ----
output <- paste0("//PHI_conf/MentalHealth1/Inpatient care/Publications/Hospital-Inpatient-Care-of-People-with-Mental-Health-Problems-in-Scotland/",
                  pub_date, "/Data/")
...
suppressWarnings(channel <- dbConnect(odbc(), dsn="SMRA",
...
# Define SQL query 
Query_SMR01 <- paste("SELECT LINK_NO, ADMISSION_DATE, DISCHARGE_DATE, CIS_MARKER, DOB, SEX,",
...
Query_SMR04 <- paste("SELECT LINK_NO, ADMISSION_DATE, DISCHARGE_DATE, CIS_MARKER, DOB, SEX,",
...
Query_SMR04_historic <- paste("SELECT LINK_NO, ADMISSION_DATE, DISCHARGE_DATE, CIS_MARKER, DOB,",
...
data_historic <- data_historic %>%
      rename(MAIN_CONDITION    = OLD_DISCHARGE_DIAG_1,
...
data_smr01 <- data_smr01 %>% 
        mutate(fyear = paste0(ifelse(month(discharge_date) >= 4, year(discharge_date), year(discharge_date)-1),
                      "/",
                      ifelse(month(discharge_date) >= 4, year(discharge_date) + 1, year(discharge_date))),
               fyear_pt1_dis = substr(fyear, 1, 4),
               adm_fyear = paste0(ifelse(month(admission_date) >= 4, year(admission_date), year(admission_date)-1),
                      "/",
...
# Hospital Types
data_smr04 <- data_smr04 %>% 
  mutate(hospital_type = ifelse(is.na(hospital_type) == T, 0, hospital_type))
...
data_res[admission_date <= ymd('19970331') & discharge_date >= ymd('19970401'), 
           r1996_1997 := 1]
data_res[admission_date <= ymd('19980331') & discharge_date >= ymd('19980401'), 
           r1997_1998 := 1]
data_res[admission_date <= ymd('19990331') & discharge_date >= ymd('19990401'), 
         r1998_1999 := 1]
data_res[admission_date <= ymd('20000331') & discharge_date >= ymd('20000401'), 
         r1999_2000:= 1]
data_res[admission_date <= ymd('20010331') & discharge_date >= ymd('20010401'), 
         r2000_2001:= 1]
data_res[admission_date <= ymd('20020331') & discharge_date >= ymd('20020401'), 
         r2001_2002:= 1]
data_res[admission_date <= ymd('20030331') & discharge_date >= ymd('20030401'), 
         r2002_2003:= 1]
data_res[admission_date <= ymd('20040331') & discharge_date >= ymd('20040401'), 
         r2003_2004:= 1]
data_res[admission_date <= ymd('20050331') & discharge_date >= ymd('20050401'), 
         r2004_2005:= 1]
data_res[admission_date <= ymd('20060331') & discharge_date >= ymd('20060401'), 
         r2005_2006:= 1]
data_res[admission_date <= ymd('20070331') & discharge_date >= ymd('20070401'), 
         r2006_2007:= 1]
data_res[admission_date <= ymd('20080331') & discharge_date >= ymd('20080401'), 
         r2007_2008:= 1]
data_res[admission_date <= ymd('20090331') & discharge_date >= ymd('20090401'), 
         r2008_2009:= 1]
data_res[admission_date <= ymd('20100331') & discharge_date >= ymd('20100401'), 
         r2009_2010:= 1]
data_res[admission_date <= ymd('20110331') & discharge_date >= ymd('20110401'), 
         r2010_2011:= 1]
data_res[admission_date <= ymd('20120331') & discharge_date >= ymd('20120401'), 
         r2011_2012:= 1]
data_res[admission_date <= ymd('20130331') & discharge_date >= ymd('20130401'), 
         r2012_2013:= 1]
data_res[admission_date <= ymd('20140331') & discharge_date >= ymd('20140401'), 
         r2013_2014:= 1]
data_res[admission_date <= ymd('20150331') & discharge_date >= ymd('20150401'), 
         r2014_2015:= 1]
data_res[admission_date <= ymd('20160331') & discharge_date >= ymd('20160401'), 
         r2015_2016:= 1]
data_res[admission_date <= ymd('20170331') & discharge_date >= ymd('20170401'), 
         r2016_2017:= 1]
data_res[admission_date <= ymd('20180331') & discharge_date >= ymd('20180401'), 
         r2017_2018:= 1]
data_res[admission_date <= ymd('20190331') & discharge_date >= ymd('20190401'), 
         r2018_2019:= 1]
data_res[admission_date <= ymd('20200331') & discharge_date >= ymd('20200401'), 
         r2019_2020:= 1]
data_res[admission_date <= ymd('20210331') & discharge_date >= ymd('20210401'), 
         r2020_2021:= 1]
data_res[admission_date <= ymd('20220331') & discharge_date >= ymd('20220401'), 
         r2021_2022:= 1]
...
data_smr_all <- data_smr_all %>% 
  mutate(SMR01 = ifelse(is.na(SMR01) == T, 0, SMR01),
         SMR04 = ifelse(is.na(SMR04) == T, 0, SMR04))
...
data_smr_all <- data_smr_all %>% 
  mutate(hbtreat_name = case_when(hbtreat_currentdate == "S08000015" |
