...
### 3 -  Set Filepaths ----
input <- paste0("//PHI_conf/MentalHealth1/Inpatient care/Publications/Hospital-Inpatient-Care-of-People-with-Mental-Health-Problems-in-Scotland/", 
                pub_date, "/Data/")
output <- paste0("//PHI_conf/MentalHealth1/Inpatient care/Publications/Hospital-Inpatient-Care-of-People-with-Mental-Health-Problems-in-Scotland/", 
                pub_date, "/Output/")
lookups <- paste0("//PHI_conf/MentalHealth1/Inpatient care/Publications/Hospital-Inpatient-Care-of-People-with-Mental-Health-Problems-in-Scotland/", 
                  pub_date, "/Lookups/")
...
data <- data %>% 
  ...
  mutate(specialty = ifelse(is.na(specialty) == TRUE, "NA", specialty),
...
discharge_data <- data %>%
  ...
  summarise(DischargesTotal = sum(count_var),
...
patients_data <- data %>% 
  ...
  summarise(specialty = last(specialty), 
...
patients_data <- patients_data %>% 
  ...
  summarise(count_var = max(count_var), 
            SMR01 = max(SMR01), 
            SMR04 = max(SMR04))%>%
  group_by(fyear, council_area) %>% 
  summarise(PatientsTotal = sum(count_var), 
...
data_with_pops <- data_with_pops %>% 
  mutate(DischargesTotal = plyr::round_any(DischargesTotal, 10),
...
### 8 - Calculate rates ----
data_with_pops <- data_with_pops %>% 
  mutate(DischargesTotal_Rates = round((DischargesTotal/Pop*100000), 2), 
...
data_with_pops_long <- data_with_pops %>%
  pivot_longer(c(DischargesTotal_Rates, DischargesNonPsychiatric_Rates, DischargesPsychiatric_Rates, 
