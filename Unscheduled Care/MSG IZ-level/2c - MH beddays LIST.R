...
odbcPreviewObject(channel, table = "ANALYSIS.SMR04", rowLimit = 0)

# Extract SMR04 data
smr04_extract <- as_tibble(dbGetQuery(
  channel,
  statement = "SELECT LINK_NO, ADMISSION_DATE,
                                      ...
                                      DISCHARGE, URI, INTZONE_2011 FROM ANALYSIS.SMR04_PI
                                      WHERE ADMISSION_DATE >= TO_DATE('1997-01-01','YYYY-MM-DD') AND HBRES_CURRENTDATE = 'S08000015'"
)) %>%
...
dups_2_flags <- dups_2 %>%
  ...
  mutate(
    min_admission = min(admission_date, na.rm = TRUE),
  ...
  mutate(
    open_flag = if_else(
      (is.na(discharge_date)) &
        lag(!is.na(discharge_date)) &
        admission_date < lag(discharge_date),
      1,
      0
    )
  ) %>%
...
dups_6 <- dups_5 %>%
  ...
  mutate(
    duplicate6 = if_else(
      (!is.na(discharge_date)) &
        admission_date == lag(admission_date) &
...
beddays <- smr04_no_dups %>%
  ...
  mutate(
    discharge_date = if_else(is.na(discharge_date), last_date, discharge_date)
  ) %>%
...
arrow::write_parquet(
  list_output,
  path(data_folder, "2c-MH-Beddays-breakdown.parquet"),
  compression = "zstd"
)
