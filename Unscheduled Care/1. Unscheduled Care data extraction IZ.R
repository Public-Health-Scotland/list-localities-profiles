...
write_parquet(
  msg_emergency_adm,
  path(exportfolder, "emergency_admissions_msg.parquet"),
  compression = "zstd"
)
...
write_parquet(
  msg_bed_days,
  path(exportfolder, "bed_days_msg.parquet"),
  compression = "zstd"
)
...
write_parquet(
  msg_bed_days_mh,
  path(exportfolder, "bed_days_mh_msg.parquet"),
  compression = "zstd"
)
...
write_parquet(
  msg_ae,
  path(exportfolder, "ae_attendances_msg.parquet"),
  compression = "zstd"
)
...
query_smr1 <- paste(
  "SELECT ADMISSION_DATE, ADMISSION, ADMISSION_TYPE, OLD_SMR1_TADM_CODE, CIS_MARKER, COUNCIL_AREA_2019, LOCATION, DATAZONE_2011,
                   ...
                   LINK_NO, LENGTH_OF_STAY, RECORD_TYPE,URI, SORT_MARKER, UPI_NUMBER, INTZONE_2011",
  "FROM ANALYSIS.SMR01_PI WHERE (DISCHARGE_DATE >= TO_DATE('2017-04-01','YYYY-MM-DD'))"
...
query_deaths <- paste(
  "SELECT LINK_NO, DATE_OF_DEATH,AGE, DATE_OF_BIRTH, POSTCODE, COUNCIL_AREA_2019, CHI,
                     ...
                     CAUSE_OF_DEATH_CODE_7, CAUSE_OF_DEATH_CODE_8,CAUSE_OF_DEATH_CODE_9,INTZONE_2011",
  "FROM ANALYSIS.GRO_DEATHS_C WHERE (DATE_OF_DEATH >= TO_DATE('2017-04-01','YYYY-MM-DD'))"
) # AND COUNCIL_AREA_2019 = 'S12000035')")
...
smr_falls <- smr1_extract %>%
  ...
  mutate(
    fall_flag = purrr::pmap_dbl(
      select(., contains("condition")),
      ~ any(grepl("^W[00-19]", c(...)), na.rm = TRUE) * 1
    )
  ) %>%
  filter(fall_flag == 1) %>%
  select(-fall_flag) %>%
  rename(
    datazone2011 = datazone_2011,
...
smr_readmissions <- read_dataframe %>%
  ...
  mutate(
    discharge_to_death = time_length(
      interval(discharge_date, date_of_death),
      unit = "day"
    )
  ) %>%
  mutate(
    discharge_dead = if_else(
  ...
  mutate(
    read_28 = if_else(
      link_no == lag(link_no) &
        lag(admission_type) %in%
          c(20, 21, 22, 30, 31, 32, 33, 34, 35, 36, 38, 39) &
        time_length(
          interval(discharge_date, lag(admission_date)),
          unit = "day"
        ) <=
          28,
      1,
...
ppa_id <- smr1_extract %>%
  ...
  filter(
    admission_type %in% c(30, 31, 32, 33, 34, 35, 36, 38, 39, 20, 21, 22, 18)
  ) %>% # AR - added 18 (planned transfers) and removed 37 as discovery metadata
  ...
  mutate(
    op1a13 = replace_na(op1a13, "0"),
    d1c14 = replace_na(d1c14, "0"),
    d2c14 = replace_na(d2c14, "0"),
    d3c14 = replace_na(d3c14, "0"),
    d4c14 = replace_na(d4c14, "0"),
    d5c14 = replace_na(d5c14, "0"),
    d6c14 = replace_na(d6c14, "0"),
    d1c13 = replace_na(d1c13, "0"),
    d2c13 = replace_na(d2c13, "0"),
    d3c13 = replace_na(d3c13, "0"),
    d4c13 = replace_na(d4c13, "0"),
    d5c13 = replace_na(d5c13, "0"),
    d6c13 = replace_na(d6c13, "0")
  ) %>%
  mutate(
    ent = ifelse(
      (d1c13 %in%
        c("H66", "J06") |
        d1c14 %in% c("J028", "J029", "J038", "J039", "J321")),
      1,
      0
    )
  ) %>%
  mutate(
    dent = ifelse(d1c13 %in% c("K02", "K03", "K04", "K05", "K06", "K08"), 1, 0)
  ) %>%
  ...
  mutate(
    dehyd = ifelse(
      (d1c13 == "E86" | d1c14 %in% c("K522", "K528", "K529")),
      1,
      0
    )
  ) %>%
  ...
  mutate(
    perf = ifelse(
      (d1c13 %in%
        c("K25", "K26", "K27", "K28") &
        substr(d1c14, 4, 4) %in% c("0", "1", "2", "4", "5", "6")),
      1,
      0
    )
  ) %>%
  mutate(
    cell = ifelse(
      (d1c13 %in%
        c("L03", "L04") |
        d1c14 %in% c("L080", "L088", "L089", "L980")),
      1,
      0
    )
  ) %>%
  mutate(
    cell = ifelse(
      cell == 1 & op1a13 %in% c("S06", "S57", "S68", "S70", "W90", "X11"),
      0,
      cell
    )
  ) %>%
  ...
  mutate(
    flu = ifelse(
      (d1c13 %in%
        c("J10", "J11", "J13") |
        d2c13 %in% c("J10", "J11", "J13") |
        d3c13 %in% c("J10", "J11", "J13") |
        d4c13 %in% c("J10", "J11", "J13") |
        d5c13 %in% c("J10", "J11", "J13") |
        d6c13 %in% c("J10", "J11", "J13") |
        d1c14 == "J181" |
        d2c14 == "J181" |
        d3c14 == "J181" |
        d4c14 == "J181" |
  ...
  mutate(
    othvacc = ifelse(
      (d1c13 %in%
        c("A35", "A36", "A80", "B05", "B06", "B26") |
        d2c13 %in% c("A35", "A36", "A80", "B05", "B06", "B26") |
        d3c13 %in% c("A35", "A36", "A80", "B05", "B06", "B26") |
        d4c13 %in% c("A35", "A36", "A80", "B05", "B06", "B26") |
        d5c13 %in% c("A35", "A36", "A80", "B05", "B06", "B26") |
        d6c13 %in% c("A35", "A36", "A80", "B05", "B06", "B26") |
        d1c14 %in% c("A370", "A379", "B161", "B169") |
        d2c14 %in% c("A370", "A379", "B161", "B169") |
        d3c14 %in% c("A370", "A379", "B161", "B169") |
        d4c14 %in% c("A370", "A379", "B161", "B169") |
        d5c14 %in% c("A370", "A379", "B161", "B169") |
        d6c14 %in% c("A370", "A379", "B161", "B169")),
      1,
      0
    )
  ) %>%
  ...
  mutate(
    diabetes = ifelse(
      (d1c13 == "E10" & d1c14 != "E109") |
        (d1c13 == "E11" & d1c14 != "E119") |
        (d1c13 == "E12" & d1c14 != "E129") |
        (d1c13 == "E13" & d1c14 != "E139") |
        (d1c13 == "E14" & d1c14 != "E149") |
        (d2c13 == "E10" & d2c14 != "E109") |
        (d2c13 == "E11" & d2c14 != "E119") |
        (d2c13 == "E12" & d2c14 != "E129") |
        (d2c13 == "E13" & d2c14 != "E139") |
        (d2c13 == "E14" & d2c14 != "E149") |
        (d3c13 == "E10" & d3c14 != "E109") |
        (d3c13 == "E11" & d3c14 != "E119") |
        (d3c13 == "E12" & d3c14 != "E129") |
        (d3c13 == "E13" & d3c14 != "E139") |
        (d3c13 == "E14" & d3c14 != "E149") |
        (d4c13 == "E10" & d4c14 != "E109") |
        (d4c13 == "E11" & d4c14 != "E119") |
        (d4c13 == "E12" & d4c14 != "E129") |
        (d4c13 == "E13" & d4c14 != "E139") |
        (d4c13 == "E14" & d4c14 != "E149") |
        (d5c13 == "E10" & d5c14 != "E109") |
        (d5c13 == "E11" & d5c14 != "E119") |
        (d5c13 == "E12" & d5c14 != "E129") |
        (d5c13 == "E13" & d5c14 != "E139") |
        (d5c13 == "E14" & d5c14 != "E149") |
        (d6c13 == "E10" & d6c14 != "E109") |
        (d6c13 == "E11" & d6c14 != "E119") |
        (d6c13 == "E12" & d6c14 != "E129") |
        (d6c13 == "E13" & d6c14 != "E139") |
        (d6c13 == "E14" & d6c14 != "E149"),
      1,
      0
    )
  ) %>%
  ...
  mutate(
    hypert = ifelse(
      (hypert == 1 &
        (op1a13 %in% range("K01", "K50") | op1a13 %in% c("K56", "K60", "K61"))),
      0,
      hypert
    )
  ) %>%
  ...
  mutate(
    angina = ifelse(
      (angina == 1 & op1a13 %in% c("K40", "K45", "K49", "K60", "K65", "K66")),
      0,
      angina
    )
  ) %>%
  mutate(
    copd = ifelse(
      (d1c13 %in% c("J41", "J42", "J43", "J44")) |
        (d1c13 == "J20" & d2c13 %in% c("J41", "J42", "J43", "J44")),
      1,
      0
    )
  ) %>%
  mutate(
    chf = ifelse((d1c13 == "I50" | d1c13 == "J81" | d1c14 == "I110"), 1, 0)
  ) %>%
  mutate(
    chf = ifelse(
      chf == 1 &
...
ppa <- ppa_id %>%
  ...
  ungroup() %>%
  mutate(
    ent = ifelse(ent > 0, 1, 0),
    dent = ifelse(dent > 0, 1, 0),
    conv = ifelse(conv > 0, 1, 0),
    gang = ifelse(gang > 0, 1, 0),
    nutridef = ifelse(nutridef > 0, 1, 0),
    dehyd = ifelse(dehyd > 0, 1, 0),
    pyelon = ifelse(pyelon > 0, 1, 0),
    perf = ifelse(perf > 0, 1, 0),
    cell = ifelse(cell > 0, 1, 0),
    pelvic = ifelse(pelvic > 0, 1, 0),
    flu = ifelse(flu > 0, 1, 0),
    othvacc = ifelse(othvacc > 0, 1, 0),
    iron = ifelse(iron > 0, 1, 0),
    asthma = ifelse(asthma > 0, 1, 0),
    diabetes = ifelse(diabetes > 0, 1, 0),
    hypert = ifelse(hypert > 0, 1, 0),
    angina = ifelse(angina > 0, 1, 0),
    copd = ifelse(copd > 0, 1, 0),
    chf = ifelse(chf > 0, 1, 0)
  ) %>%
  group_by(link_no, cis_marker) %>%
  summarise(
    ent = first(ent),
...
write_parquet(
  smr_falls,
  path(exportfolder, "falls_smr.parquet"),
  compression = "zstd"
)
...
write_parquet(
  smr_readmissions,
  path(exportfolder, "readmissions_smr.parquet"),
  compression = "zstd"
)
